...
function DashboardPanel(props: DashboardPanelProps): JSX.Element {
  const toneClasses =
    props.accent === "amber"
      ? "border-secondary/30 bg-amber-300/[0.07]"
      : props.accent === "rose"
        ? "border-destructive/20 bg-rose-300/[0.07]"
        : "border-border bg-card";

  return (
    <section
      className={cn(
        "rounded-[1.8rem] border px-5 py-5 shadow-[0_24px_80px_-52px_rgba(15,23,42,0.95)] ",
        toneClasses,
      )}
    >
...
function SummaryCard(props: SummaryCardProps): JSX.Element {
  ...
  const iconClasses =
    props.tone === "critical"
      ...
      : props.tone === "warning"
        ? "border-secondary/30 bg-secondary/20 text-secondary-foreground"
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 text-sm leading-6 text-muted-foreground">{props.description}</p>
    </article>
  );
}
...
export function DashboardSummary(props: DashboardSummaryProps): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[1.8rem] border border-border bg-card px-5 py-4 shadow-[0_24px_80px_-52px_rgba(15,23,42,0.95)] ">
        ...
        {refreshError ? (
          <div className="mt-4 rounded-[1.25rem] border border-destructive/20 bg-destructive/10 px-4 py-3 text-sm leading-6 text-destructive">
            {refreshError}
          </div>
        ) : null}
      </section>
          ...
          >
            <div className="overflow-hidden rounded-[1.35rem] border border-border">
              {data.tables.length === 0 ? (
                <div className="bg-card px-4 py-6 text-sm text-muted-foreground">
                  卓データがまだ作成されていません。
                </div>
              ) : (
                <div className="divide-y divide-white/10">
                  {data.tables.map((table) => (
                    <div
                      key={table.tableId}
                      className="grid gap-3 bg-card px-4 py-4 lg:grid-cols-[auto_1fr_auto]"
                    >
                      <div>
            ...
            <div className="space-y-3">
              {data.disconnectedParticipants.length === 0 ? (
                ...
                data.disconnectedParticipants.map((participant) => (
                  <article
                    key={participant.participantId}
                    className="rounded-[1.35rem] border border-destructive/20 bg-destructive/10 px-4 py-4"
                  >
            ...
            <div className="space-y-3">
              {data.disputedMatches.length === 0 ? (
                ...
                data.disputedMatches.map((match) => (
                  <article
                    key={match.matchId}
                    className="rounded-[1.35rem] border border-secondary/30 bg-secondary px-4 py-4"
                  >
                    <p className="text-sm font-semibold text-foreground">
                      {match.tableNumber ? `${match.tableNumber} 卓` : "卓未割当"}
                    </p>
