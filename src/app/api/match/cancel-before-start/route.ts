...
import { executeCancelBeforeStart } from "@/lib/services/match-service";
import { cancelMatchBeforeStartSchema } from "@/lib/validators/match";

export async function POST(request: Request): Promise<Response> {
  try {
    const body = await request.json();
    const input = cancelMatchBeforeStartSchema.parse(body);
    ...
    const runtime = await executeCancelBeforeStart({
      participantId,
