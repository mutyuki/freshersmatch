...
import { adjustParticipantChip } from "@/lib/services/admin-participant-service";
import { adminAdjustParticipantChipSchema } from "@/lib/validators/admin";

export async function POST(request: Request): Promise<Response> {
  try {
    const input = adminAdjustParticipantChipSchema.parse(await request.json());
    ...
    const result = await adjustParticipantChip({
      adminUserId,
