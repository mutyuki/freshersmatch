...
declare
  ...
  v_has_match_reference boolean := false;
begin
  select id
  into v_admin_id
  ...
  if v_participant.status not in ('registered', 'paused', 'disqualified', 'unregistered') then
    raise exception 'Participant cannot be deleted from status: %', v_participant.status;
  end if;
  ...
  into v_has_match_reference;

  if v_has_match_reference then
    raise exception 'Participant cannot be deleted because match history exists: %', p_participant_id;
  end if;
