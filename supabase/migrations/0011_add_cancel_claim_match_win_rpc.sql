create or replace function public.cancel_claim_match_win(
...
begin
  ...
  if v_match_locked.is_staff_match then
    raise exception 'Staff matches cannot cancel participant win claims: %', p_match_id;
  end if;
  ...
  if v_match_locked.winner_claimed_by_participant_id <> p_participant_id then
    raise exception 'Only the claiming participant can cancel the winner claim: %', p_participant_id;
  end if;
